namespace Streebog
{
    static public class Streebog
    {
        private const int inputSize = 64;
        private static readonly byte[] nullBytes = Enumerable.Repeat(0x00, inputSize).Select(x => (byte)x).ToArray();
        ...
        public static byte[] HashFunc(byte[] data, bool outputSize, bool inputType)
        {
            //если входной формат hex - перевод в байтовый вид
            if (inputType == false)
            {
                int temp = data.Length / 2;
                for (int i = 0; i < temp; i++)              
                    data[i] = Convert.ToByte(((char)data[2 * i]).ToString() + (char)data[2 * i + 1], 16);                
                data = data.Take(temp).ToArray();
            }

            int n = data.Length / inputSize;
            byte[] h, N, Sigma;
            h = new byte[inputSize];
            N = new byte[inputSize];
            Sigma = new byte[inputSize];
            // нулевые байты
            if (outputSize == false)          
                nullBytes.CopyTo(h, 0);   
            //единичные байты
            else          
                h = Enumerable.Repeat(0x01, inputSize).Select(x => (byte)x).ToArray();
            byte[] m;
            //Обработка блоков по 512 бит
            for (int i = 1; i <= n; i++)
            {
                m = new byte[inputSize];
                data.TakeLast(inputSize).ToArray().CopyTo(m, 0);
                h = gFunc(N, h, m);
                byte[] numberInBytes = new byte[inputSize];
                ...
                data = data.SkipLast(i * inputSize).ToArray();
            }
            //Дополнение последнего блока до 512 бит
            m = new byte[inputSize];
            m[inputSize - data.Length - 1] = 1;
            Array.Copy(data, 0, m, inputSize - data.Length, data.Length);
            //Обработка последнего блока
            h = gFunc(N, h, m);
            byte[] lengthInBytes = new byte[inputSize];
        ...
        // Линейое преобразование
        private static byte[] LFunc(byte[] K)
        {
            byte[] result = new byte[inputSize];
            for (int i = 0; i < 8; i++)
            {
                ...
                for (int j = 0; j < 8; j++)
                {
                    currentPart += (ulong)K[i * 8 + j] << (inputSize - (j + 1) * 8); //Переводим 8 байт массива в длинное 8-байтовое целое
                }
                for (int j = 0; j < 64; j++)
                {
                    if ((currentPart >> j & 1) == 1)
                        currentResult ^= A[inputSize - j - 1];
                }
                for (int j = 0; j < 8; j++)
                {
                    result[i * 8 + j] = (byte)(currentResult >> (inputSize - (j + 1) * 8) & 0xFF); //Из 8-байтового целого в массив из 8 байт
                }
            }
            return result;
        }
